...
type ContactNameKey = "emailGlobal" | "emailCn" | "zhihu" | "weibo" | "douban"

const data: {
  categoryId: "social" | "im"
  items: {
...
export const ContactSection: React.FunctionComponent<IPropsContactSection> = (
  ...
  const itemDisplayName = (item: (typeof data)[0]["items"][0]) =>
    item.nameKey ? t(`itemNames.${item.nameKey}`) : item.name
            ...
            <Space className={styles.list} size={8} wrap>
              {category.items.map((item) => (
                <Popover
                  ...
                  title={
                    <Space>
                      {item.icon} {itemDisplayName(item)}
                    </Space>
                  }
                  content={
