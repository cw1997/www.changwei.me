...
export const GuestbookList: React.FunctionComponent<IPropsGuestbookList> = (props) => {
  const [offset, setOffset] = useState(0)
  const [limit, setLimit] = useState(20)
  const swr_list = useSWR(['guestbook/list', offset, limit], () => fetch('/api/guestbook/list', {
    method: 'POST',
  ...
  const jsx_pagination = data && (
    <Pagination
      pageSize={limit} current={offset / limit + 1} total={data.data.count} responsive
      showQuickJumper showSizeChanger showTitle showPrevNextJumpers showTotal={(total) => `Total records ${total}`}
  ...
  return (
    <div className={styles.container}>
      <h3 className={styles.title}>Threads {swr_list.isValidating ? <LoadingOutlined /> : `(${data?.data.count})`}</h3>
          ...
          {data?.data.items.map((row: any) => (
            <div key={row.id} className={styles.item}>
              <div className={styles.item_info}>
                <Space split={<Divider type={'vertical'} />} className={styles.item_info}>
